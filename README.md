...
```bash
curl -fsSL https://raw.githubusercontent.com/Moontariun/Tumacord/release/autoridade-do-servidor-v0.9.4/scripts/install-v0.9.4.sh | bash
```

Este comando instala a v0.9.4 a partir da branch separada `release/autoridade-do-servidor-v0.9.4`. As versões anteriores permanecem isoladas em suas próprias branches e não devem mais ser usadas.
...
Baixe `Tumacord-0.9.4-Setup.exe` nas [Releases](https://github.com/Moontariun/Tumacord/releases)
...
`Tumacord-0.9.4-portable.exe` roda sem instalar nada. É a opção para pendrive
...
```powershell
Get-AuthenticodeSignature .\Tumacord-0.9.4-Setup.exe | Format-List Status, SignerCertificate
```
