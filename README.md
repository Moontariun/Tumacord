...
```bash
curl -fsSL https://raw.githubusercontent.com/Moontariun/Tumacord/release/traco-e-exclusao-v0.9.3/scripts/install-v0.9.3.sh | bash
```

Este comando instala a v0.9.3 a partir da branch separada `release/traco-e-exclusao-v0.9.3`. As versões anteriores permanecem isoladas em suas próprias branches e não devem mais ser usadas.
...
Baixe `Tumacord-0.9.3-Setup.exe` nas [Releases](https://github.com/Moontariun/Tumacord/releases)
...
`Tumacord-0.9.3-portable.exe` roda sem instalar nada. É a opção para pendrive
...
```powershell
Get-AuthenticodeSignature .\Tumacord-0.9.3-Setup.exe | Format-List Status, SignerCertificate
```
