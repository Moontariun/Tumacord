...
```bash
curl -fsSL https://raw.githubusercontent.com/Moontariun/Tumacord/release/isolamento-de-origem-v0.9.5/scripts/install-v0.9.5.sh | bash
```

Este comando instala a v0.9.5 a partir da branch separada `release/isolamento-de-origem-v0.9.5`. As versões anteriores permanecem isoladas em suas próprias branches e não devem mais ser usadas.
...
Baixe `Tumacord-0.9.5-Setup.exe` nas [Releases](https://github.com/Moontariun/Tumacord/releases)
...
`Tumacord-0.9.5-portable.exe` roda sem instalar nada. É a opção para pendrive
...
```powershell
Get-AuthenticodeSignature .\Tumacord-0.9.5-Setup.exe | Format-List Status, SignerCertificate
```
