...
```bash
curl -fsSL https://raw.githubusercontent.com/Moontariun/Tumacord/release/bandeja-e-correcoes-v0.9.2/scripts/install-v0.9.2.sh | bash
```

Este comando instala a v0.9.2 a partir da branch separada `release/bandeja-e-correcoes-v0.9.2`. As versões anteriores permanecem isoladas em suas próprias branches e não devem mais ser usadas.
...
Baixe `Tumacord-0.9.2-Setup.exe` nas [Releases](https://github.com/Moontariun/Tumacord/releases)
...
`Tumacord-0.9.2-portable.exe` roda sem instalar nada. É a opção para pendrive
...
```powershell
Get-AuthenticodeSignature .\Tumacord-0.9.2-Setup.exe | Format-List Status, SignerCertificate
```
