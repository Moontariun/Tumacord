...
alvo="${1:-release/bandeja-e-correcoes-v0.9.2}"
...
if [[ "$alvo" == "ultima" || "$alvo" == "--ultima" ]]; then
  ...
  if [[ -z "$alvo" ]]; then
    ...
    echo "Rode de novo com a referência: ./scripts/update-server.sh release/bandeja-e-correcoes-v0.9.2" >&2
    exit 1
  fi
...
echo "  git checkout release/mesa-de-desenho-v0.9.1 && docker compose up -d --build" >&2
echo >&2
