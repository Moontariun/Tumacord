...
alvo="${1:-release/isolamento-de-origem-v0.9.5}"
...
if [[ "$alvo" == "ultima" || "$alvo" == "--ultima" ]]; then
  ...
  if [[ -z "$alvo" ]]; then
    ...
    echo "Rode de novo com a referência: ./scripts/update-server.sh release/isolamento-de-origem-v0.9.5" >&2
    exit 1
  fi
...
echo "  git checkout release/autoridade-do-servidor-v0.9.4 && docker compose up -d --build" >&2
echo >&2
