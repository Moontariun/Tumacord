...
export async function loadLocalSyncBundle(origin: string): Promise<ChatSyncBundle> {
  if (!window.tumacordDesktop || !origin) return emptyBundle();
  try {
    const response = await fetch(`${LOCAL_SERVER}/api/local/sync?origin=${encodeURIComponent(origin)}`);
...
export async function mirrorLocally(origin: string, channels: Channel[], messages: ChatMessage[], profiles: ReplicatedProfile[] = []): Promise<void> {
  if (!window.tumacordDesktop || !origin || (!channels.length && !messages.length && !profiles.length)) return;
