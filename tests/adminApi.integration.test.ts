...
// Ligar o relay pelo painel, sem editar arquivo e reiniciar contêiner.
test('o relay é configurado pelo painel, e o segredo nunca volta', { timeout: 30_000 }, async (context) => {
  const { url, dono, comum } = await servidor(context);

  const inicial = await (await fetch(`${url}/api/admin/settings`, comoAdmin(dono.token))).json() as { turn: { urls: string[]; secretConfigured: boolean; managedBy: string } };
  assert.deepEqual(inicial.turn.urls, []);
  assert.equal(inicial.turn.secretConfigured, false);
  assert.equal(inicial.turn.managedBy, 'nenhum');

  const segredo = 'segredo-do-relay-bem-comprido';
  const salvo = await fetch(`${url}/api/admin/settings`, comoAdmin(dono.token, 'PATCH', {
    turnUrls: ['turn:turn.exemplo.com:3478'], turnSecret: segredo, turnTtlSeconds: 3600,
  }));
  assert.equal(salvo.status, 200);
  const corpo = await salvo.text();
  assert.equal(corpo.includes(segredo), false, 'o segredo não pode voltar na resposta');
  const depois = JSON.parse(corpo) as { turn: { urls: string[]; secretConfigured: boolean; ttlSeconds: number; managedBy: string } };
  assert.deepEqual(depois.turn.urls, ['turn:turn.exemplo.com:3478']);
  assert.equal(depois.turn.secretConfigured, true);
  assert.equal(depois.turn.ttlSeconds, 3600);
  assert.equal(depois.turn.managedBy, 'painel');

  // E o relay passa a valer de verdade, sem reiniciar nada.
  const saude = await (await fetch(`${url}/api/health`)).json() as { turn: boolean; capabilities: { turn: boolean } };
  assert.equal(saude.turn, true);
  assert.equal(saude.capabilities.turn, true);

  const credenciais = await (await fetch(`${url}/api/turn`, comoAdmin(comum.token))).json() as { iceServers: Array<{ urls: string[]; credential: string }> };
  assert.deepEqual(credenciais.iceServers[0].urls, ['turn:turn.exemplo.com:3478']);
  assert.notEqual(credenciais.iceServers[0].credential, segredo, 'a credencial é derivada, não o segredo em si');

  // Nem o registro de auditoria pode carregar o valor.
  const registro = await (await fetch(`${url}/api/admin/audit`, comoAdmin(dono.token))).text();
  assert.equal(registro.includes(segredo), false, 'um segredo em log de auditoria é um segredo vazado');
});

test('a configuração do relay é validada, e usuário comum não a alcança', { timeout: 30_000 }, async (context) => {
  const { url, dono, comum } = await servidor(context);
  assert.equal((await fetch(`${url}/api/admin/settings`, comoAdmin(comum.token))).status, 403);
  assert.equal((await fetch(`${url}/api/admin/settings`, comoAdmin(comum.token, 'PATCH', { turnSecret: 'tentativa-de-invasao' }))).status, 403);

  for (const [corpo, motivo] of [
    [{ turnUrls: ['http://nao-e-turn'] }, 'endereço que não é TURN'],
    [{ turnSecret: 'curto' }, 'segredo curto demais'],
    [{ turnTtlSeconds: 10 }, 'validade curta demais'],
    [{ turnTtlSeconds: 999999 }, 'validade longa demais'],
  ] as Array<[unknown, string]>) {
    const resposta = await fetch(`${url}/api/admin/settings`, comoAdmin(dono.token, 'PATCH', corpo));
    assert.equal(resposta.status, 400, `deveria recusar: ${motivo}`);
  }
});
